...
func genMaterials(matData) -> Array:
	var matObject:Array[StandardMaterial3D]
	# Check if a Godot material already exists
	for mat in matData:
		var godotMatFilePath = "res://materials/" + mat + ".tres"
		var godotMat = mat + ".tres"
		
		if not mat == "":
			var matFilePath = "res://materials/" + mat + ".rdm"
			## Disabled this so that we can still generate the Godot materials for reference, 
			## but still be able to regenerate them whenever the game loads in the event the .rdm
			## file has had changes.
		...
		for i in mats:
			matObject.append(ResourceLoader.load("res://materials/" + mat + ".tres"))
	#print(mats)
	return mats
